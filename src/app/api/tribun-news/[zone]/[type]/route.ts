...
export async function GET(
  request: NextRequest,
  { params }: { params: { zone: string; type: string } }
) {
  try {
    ...
    const result = await parseRSS({
      url: TRIBUN_NEWS_RSS.replace("{zone}", params.zone).replace(
        "{type}",
        params.type
      ),
    });
    ...
    let responseData: ResponseData = {
      message: `Result of zone ${params.zone} and type ${params.type} news in Tribun News`,
      total: data.length,
      data,
    };
    ...
    if (searchParams) {
      ...
      responseData = {
        message: `Result of zone ${params.zone} news and type ${params.type} in Tribun News with title search: ${searchParams}`,
