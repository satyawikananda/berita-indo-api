...
export async function GET(
  ...
  try {
    ...
    const result = await parseRSS({
      url: CNBC_NEWS_RSS.replace("{type}", params.type),
    });
    ...
    let responseData: ResponseData = {
      message: `Result of type ${params.type} news in CNBC News`,
      total: data.length,
      data,
    };
    ...
    if (searchParams) {
      ...
      responseData = {
        message: `Result of type ${params.type} news in CNBC News with title search: ${searchParams}`,
